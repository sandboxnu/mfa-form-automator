...
import { CreateSignatureChainDto, SignatureChainLinkDto } from "./signatureChain.dto";

@Injectable()
export class SignatureChainService {
  constructor(
    @InjectRepository(SignatureChainLink) private signatureChainLinkRepository: Repository<SignatureChainLink>,
  ) {}
  ...
  async createSignatureChain(createSignatureChainDto: CreateSignatureChainDto) {
    ...
    for (var createSignatureChainLinkDto of createSignatureChainDto.signatureChainLinks) {
        ...
        const signatureChainLink: SignatureChainLink = this.signatureChainLinkRepository.create(signatureChainLinkDto);
        lastSignatureChainLink = await this.signatureChainLinkRepository.save(signatureChainLink);
